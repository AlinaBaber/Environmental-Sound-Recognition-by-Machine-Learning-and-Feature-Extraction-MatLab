function varargout = ESR(varargin)
% ESR MATLAB code for ESR.fig
%      ESR, by itself, creates a new ESR or raises the existing
%      singleton*.
%
%      H = ESR returns the handle to a new ESR or the handle to
%      the existing singleton*.
%
%      ESR('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in ESR.M with the given input arguments.
%
%      ESR('Property','Value',...) creates a new ESR or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before ESR_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to ESR_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help ESR

% Last Modified by GUIDE v2.5 29-Apr-2018 21:48:34

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @ESR_OpeningFcn, ...
                   'gui_OutputFcn',  @ESR_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before ESR is made visible.
function ESR_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to ESR (see VARARGIN)

% Choose default command line output for ESR
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes ESR wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = ESR_OutputFcn(hObject, eventdata, handles) 
...
set(handles.feature_text,'string',' Average Pitch Contour:');
...
set(handles.feature_text,'string','Formant Frequency :');
...
set(handles.feature_text,'string','Average Intensity:');
...
legend('Intensity');
zlabel(' Intensity of Sound (Watt/m^2');
...
set(handles.feature_text,'string','Rate of Acceleration');
...
wav_file=handles.sample_filepath_txt.String;
p=128;
[LPCCs_Coeff,LPCSs,LPCCscepDpDD]= lpccs(wav_file,p);
...
wav_file=handles.sample_filepath_txt.String;
[Frequency,t,x,f,magnitude,q,amplitude] = frequency( wav_file );
set(handles.feature_text,'string',' Frequency :');
...
zlabel(['Amplitude'],'FontSize',14,'Color','b');
legend('product');
...
handles.activex6.URL = FullFilePath;
data = getappdata(gcbf, 'metricdata');
...
set(handles.feature_text,'string','Pitch Frequency:');
...
% handles    structure with handles and user data (see GUIDATA)
 wav_file=handles.sample_filepath_txt.String;
[Category,Category_score,Categories_error,Category_cost,Emotion,Emotion_score,Emotion_Cost,Emotion_error] = svm_classification(wav_file);
C = get(handles.test_table, 'data');
Category_Score=num2cell(max(Category_score));
Categories_Error=num2cell(Categories_error);
Emotion_Score=num2cell(max(Emotion_score)/12);
Emotion_Error=num2cell((Emotion_error)/12);

C(1,1)= Category;%assign a cell array
C(1,2)= Category_Score;
C(1,3)= Categories_Error;
C(1,4)= {Emotion};
C(1,5)= Emotion_Score;
C(1,6)= Emotion_Error;
set(handles.test_table, 'data', C);
axes(handles.spectral_graph)
bar33(Category_score);
zlabel(' SVM Score');
...
bar33(Emotion_score);
zlabel(' SVM Score');
xlabel('(1) Angry (2) Happy (3) Neutral (4) Sad'); 
title('SVM Emotions Result');
...
C(1,1)= Category(1);%assign a cell array
C(2,1)= Category(2);%assign a cell array
C(3,1)= Category(3);%assign a cell array
C(4,1)= Category(4);%assign a cell array
C(5,1)= Category(5);%assign a cell array
C(6,1)= Category(6);%assign a cell array
set(handles.test_table, 'data', C);
axes(handles.spectral_graph)
bar3(Category_score(1,:));
zlabel(' KNN Score');
xlabel(' Categories from 1 to 49'); 
title('Spectral Analysis Result');
axes(handles.frequency_graph)
bar3(Category_score(2,:));
zlabel(' KNN Score');
xlabel(' Categories from 1 to 49'); 
title('Frequency Domain Analysis Result');
axes(handles.predictive_graph)
bar3(Category_score(3,:));
zlabel(' KNN Score');
xlabel(' Categories from 1 to 49'); 
title('Predictive Analysis Result');
axes(handles.shortterm_graph)
bar3(Category_score(4,:));
zlabel(' KNN Score');
xlabel(' Categories from 1 to 49'); 
title('Short Term Analysis Result');
axes(handles.cepstral_graph)
bar3(Category_score(5,:));
zlabel(' KNN Score');
xlabel(' Categories from 1 to 49'); 
title('Cepstral Analysis Result');
axes(handles.merge_graph)
bar3(Category_score(6,:));
zlabel(' KNN Score');
xlabel(' Categories from 1 to 49'); 
title('All features Analysis Result');
...
% handles    structure with handles and user data (see GUIDATA)
wav_file=handles.sample_filepath_txt.String;
[Category,Category_Cost,Category_Score,Category_Error,DTW_Categories,Emotion,Emotion_Cost,Emotion_Score,Emotion_Error,DTW_Emotions] = comparison(wav_file);

C = get(handles.test_table, 'data');
Category_Score=num2cell(Category_Score);
Categories_Error=num2cell(Category_Error);
Emotion_Score=num2cell(Emotion_Score);
Emotion_Error=num2cell(Emotion_Error);
...
set(handles.test_table, 'data', C);
axes(handles.spectral_graph)
bar33(DTW_Categories);
zlabel(' DTW Distance (min for accuracy)');
xlabel(' (1) Normal  (2) Special'); 
title('DTW Language Disorder Distance');
axes(handles.emotion_graph)
bar33(DTW_Emotions);
zlabel(' DTW Distance (min for accuracy)');
xlabel('(1) Angry (2) Happy (3) Neutral (4) Sad'); 
title('DTW Emotions Distance');
...
% handles    structure with handles and user data (see GUIDATA)
wav_file=handles.sample_filepath_txt.String;
[Category,Category_cost,Category_output,Category_error,Emotion,Emotion_cost,Emotions_output,Emotion_error]=neural_network_classification(wav_file);
C = get(handles.test_table, 'data');
Category_Score=num2cell(max(Category_output));
Categories_Error=num2cell(min(Category_error));
Emotion_Score=num2cell(max(Emotions_output));
Emotion_Error=num2cell(min(Emotion_error));
...
set(handles.test_table, 'data', C);
axes(handles.spectral_graph)
bar33(Category_output);
zlabel(' NN Score');
xlabel(' (1) Normal  (2) Special'); 
title('NN Language Disorder Result');
axes(handles.emotion_graph)
bar33(Emotions_output); 
zlabel(' NN Score');
xlabel('(1) Angry (2) Happy (3) Neutral (4) Sad'); 
title('NN Emotions Result');
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
[Category_Error_Rate,Category_Accuracy_Rate,Emotion_Error_Rate,Emotion_Accuracy_Rate,Categories,Emotions]= NN_Test();
C = get(handles.algo_category_table, 'data');
C(3,1)= num2cell(Category_Error_Rate);%assign a cell array
C(3,2)= num2cell(Category_Accuracy_Rate);
set(handles.algo_category_table, 'data', C);
...
[Normal_Error_Rate,Special_Error_Rate,Normal_Accuracy_Rate,Special_Accuracy_Rate]= algorithm_analysis(Categories,Emotions);
C = get(handles.error_table, 'data');
C(3,1)= num2cell(Normal_Error_Rate(1,1));%assign a cell array
C(3,2)= num2cell(Normal_Error_Rate(1,2));
C(3,3)= num2cell(Normal_Error_Rate(1,3));
C(3,4)= num2cell(Normal_Error_Rate(1,4));
C(3,5)= num2cell(Special_Error_Rate(1,1));
C(3,6)= num2cell(Special_Error_Rate(1,2));
C(3,7)= num2cell(Special_Error_Rate(1,3));
C(3,8)= num2cell(Special_Error_Rate(1,4));
set(handles.error_table, 'data', C);
C = get(handles.performance_table, 'data');
C(3,1)= num2cell(Normal_Accuracy_Rate(1,1));%assign a cell array
C(3,2)= num2cell(Normal_Accuracy_Rate(1,2));
C(3,3)= num2cell(Normal_Accuracy_Rate(1,3));
C(3,4)= num2cell(Normal_Accuracy_Rate(1,4));
C(3,5)= num2cell(Special_Accuracy_Rate(1,1));
C(3,6)= num2cell(Special_Accuracy_Rate(1,2));
C(3,7)= num2cell(Special_Accuracy_Rate(1,3));
C(3,8)= num2cell(Special_Accuracy_Rate(1,4));
set(handles.performance_table, 'data', C);
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
[Category_Error_Rate,Category_Accuracy_Rate,Emotion_Error_Rate,Emotion_Accuracy_Rate,Categories,Emotions]= SVM_Test();
C = get(handles.algo_category_table, 'data');
C(1,1)= num2cell(Category_Error_Rate);%assign a cell array
C(1,2)= num2cell(Category_Accuracy_Rate);
set(handles.algo_category_table, 'data', C);
E = get(handles.algo_emotion_table, 'data');
E(1,1)= num2cell(Emotion_Error_Rate);%assign a cell array
E(1,2)= num2cell(Emotion_Accuracy_Rate);
set(handles.algo_emotion_table, 'data', E);
[Normal_Error_Rate,Special_Error_Rate,Normal_Accuracy_Rate,Special_Accuracy_Rate]= algorithm_analysis(Categories,Emotions);
C = get(handles.error_table, 'data');
C(1,1)= num2cell(Normal_Error_Rate(1,1));%assign a cell array
C(1,2)= num2cell(Normal_Error_Rate(1,2));
C(1,3)= num2cell(Normal_Error_Rate(1,3));
C(1,4)= num2cell(Normal_Error_Rate(1,4));
C(1,5)= num2cell(Special_Error_Rate(1,1));
C(1,6)= num2cell(Special_Error_Rate(1,2));
C(1,7)= num2cell(Special_Error_Rate(1,3));
C(1,8)= num2cell(Special_Error_Rate(1,4));
set(handles.error_table, 'data', C);
C = get(handles.performance_table, 'data');
C(1,1)= num2cell(Normal_Accuracy_Rate(1,1));%assign a cell array
C(1,2)= num2cell(Normal_Accuracy_Rate(1,2));
C(1,3)= num2cell(Normal_Accuracy_Rate(1,3));
C(1,4)= num2cell(Normal_Accuracy_Rate(1,4));
C(1,5)= num2cell(Special_Accuracy_Rate(1,1));
C(1,6)= num2cell(Special_Accuracy_Rate(1,2));
C(1,7)= num2cell(Special_Accuracy_Rate(1,3));
C(1,8)= num2cell(Special_Accuracy_Rate(1,4));
set(handles.performance_table, 'data', C);
...
[Category_Error_Rate,Category_Accuracy_Rate]= KNN_Test();
C = get(handles.algo_category_table, 'data');
C(1,1)= num2cell(Category_Error_Rate(1));%assign a cell array
C(1,2)= num2cell(Category_Accuracy_Rate(1));
C(2,1)= num2cell(Category_Error_Rate(2));%assign a cell array
C(2,2)= num2cell(Category_Accuracy_Rate(2));
C(3,1)= num2cell(Category_Error_Rate(3));%assign a cell array
C(3,2)= num2cell(Category_Accuracy_Rate(3));
C(4,1)= num2cell(Category_Error_Rate(4));%assign a cell array
C(4,2)= num2cell(Category_Accuracy_Rate(4));
C(5,1)= num2cell(Category_Error_Rate(5));%assign a cell array
C(5,2)= num2cell(Category_Accuracy_Rate(5));
C(6,1)= num2cell(Category_Error_Rate(6));%assign a cell array
C(6,2)= num2cell(Category_Accuracy_Rate(6));
set(handles.algo_category_table, 'data', C);
...
% handles    structure with handles and user data (see GUIDATA)
File=load('dtwdata');
%[Category_Error_Rate,Category_Accuracy_Rate,Emotion_Error_Rate,Emotion_Accuracy_Rate,Categories,Emotions]= DTW_Test();

C = get(handles.algo_category_table, 'data');
C(4,1)= num2cell(File.Category_Error_Rate);%assign a cell array
C(4,2)= num2cell(File.Category_Accuracy_Rate);
set(handles.algo_category_table, 'data', C);
E = get(handles.algo_emotion_table, 'data');
E(4,1)= num2cell(File.Emotion_Error_Rate);%assign a cell array
E(4,2)= num2cell(File.Emotion_Accuracy_Rate);
set(handles.algo_emotion_table, 'data', E);
[Normal_Error_Rate,Special_Error_Rate,Normal_Accuracy_Rate,Special_Accuracy_Rate]= algorithm_analysis(File.Categories,File.Emotions);
C = get(handles.error_table, 'data');
C(4,1)= num2cell(Normal_Error_Rate(1,1));%assign a cell array
C(4,2)= num2cell(Normal_Error_Rate(1,2));
C(4,3)= num2cell(Normal_Error_Rate(1,3));
C(4,4)= num2cell(Normal_Error_Rate(1,4));
C(4,5)= num2cell(Special_Error_Rate(1,1));
C(4,6)= num2cell(Special_Error_Rate(1,2));
C(4,7)= num2cell(Special_Error_Rate(1,3));
C(4,8)= num2cell(Special_Error_Rate(1,4));
set(handles.error_table, 'data', C);
C = get(handles.performance_table, 'data');
C(4,1)= num2cell(Normal_Accuracy_Rate(1,1));%assign a cell array
C(4,2)= num2cell(Normal_Accuracy_Rate(1,2));
C(4,3)= num2cell(Normal_Accuracy_Rate(1,3));
C(4,4)= num2cell(Normal_Accuracy_Rate(1,4));
C(4,5)= num2cell(Special_Accuracy_Rate(1,1));
C(4,6)= num2cell(Special_Accuracy_Rate(1,2));
C(4,7)= num2cell(Special_Accuracy_Rate(1,3));
C(4,8)= num2cell(Special_Accuracy_Rate(1,4));
set(handles.performance_table, 'data', C);
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
[Category_Error_Rate,Category_Accuracy_Rate,Emotion_Error_Rate,Emotion_Accuracy_Rate,Categories,Emotions]= Hybrid_Test();
C = get(handles.algo_category_table, 'data');
...
[Normal_Error_Rate,Special_Error_Rate,Normal_Accuracy_Rate,Special_Accuracy_Rate]= algorithm_analysis(Categories,Emotions);
C = get(handles.error_table, 'data');
C(5,1)= num2cell(Normal_Error_Rate(1,1));%assign a cell array
C(5,2)= num2cell(Normal_Error_Rate(1,2));
C(5,3)= num2cell(Normal_Error_Rate(1,3));
C(5,4)= num2cell(Normal_Error_Rate(1,4));
C(5,5)= num2cell(Special_Error_Rate(1,1));
C(5,6)= num2cell(Special_Error_Rate(1,2));
C(5,7)= num2cell(Special_Error_Rate(1,3));
C(5,8)= num2cell(Special_Error_Rate(1,4));
set(handles.error_table, 'data', C);
C = get(handles.performance_table, 'data');
C(5,1)= num2cell(Normal_Accuracy_Rate(1,1));%assign a cell array
C(5,2)= num2cell(Normal_Accuracy_Rate(1,2));
C(5,3)= num2cell(Normal_Accuracy_Rate(1,3));
C(5,4)= num2cell(Normal_Accuracy_Rate(1,4));
C(5,5)= num2cell(Special_Accuracy_Rate(1,1));
C(5,6)= num2cell(Special_Accuracy_Rate(1,2));
C(5,7)= num2cell(Special_Accuracy_Rate(1,3));
C(5,8)= num2cell(Special_Accuracy_Rate(1,4));
set(handles.performance_table, 'data', C);
...
C = get(handles.test_table, 'data');
Categories_Score=num2cell(max(Category_Score)/3);
Categories_Error=num2cell(Category_Error/3);
Emotions_Score=num2cell(max(Emotion_Score)/3);
Emotion_Error=num2cell(Emotion_Error/3);
...
% --- Executes on button press in hybrid_training_btn.
function hybrid_training_btn_Callback(hObject, eventdata, handles)
% hObject    handle to hybrid_training_btn (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
Status = hybrid_training();
set(handles.training_progress_text,'string',Status);


% --- Executes on button press in compare_graph_btn.
function compare_graph_btn_Callback(hObject, eventdata, handles)
% hObject    handle to compare_graph_btn (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)[Category_Error_Rate,Category_Accuracy_Rate,Emotion_Error_Rate,Emotion_Accuracy_Rate,Categories,Emotions]= Hybrid_Test();
C = get(handles.algo_category_table, 'data');
figure(1),subplot(2,1,1);
bar333(C(1:5));
hold on;
E = get(handles.algo_emotion_table, 'data');
subplot(2,1,2);bar333(E(1:5,:)); hold off;
Error = get(handles.error_table, 'data');
figure(2),subplot(2,1,1);
bar333(Error(1:5,:));
hold on;
Performance = get(handles.performance_table, 'data');
subplot(2,1,2);
bar333(Performance(1:5,:));
hold off;


% --- Executes on button press in features1_analysis_btn.
function features1_analysis_btn_Callback(hObject, eventdata, handles)
% hObject    handle to features1_analysis_btn (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% hObject    handle to hybrid_analysis_btn (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
File=load('Features1_Results');
C = get(handles.algo_category_table, 'data');
C(5,1)= num2cell(File.Category_Error_Rate);%assign a cell array
C(5,2)= num2cell(File.Category_Accuracy_Rate);
set(handles.algo_category_table, 'data', C);
E = get(handles.algo_emotion_table, 'data');
E(5,1)= num2cell(File.Emotion_Error_Rate);%assign a cell array
E(5,2)= num2cell(File.Emotion_Accuracy_Rate);
set(handles.algo_emotion_table, 'data', E);
[Normal_Error_Rate,Special_Error_Rate,Normal_Accuracy_Rate,Special_Accuracy_Rate]= algorithm_analysis(File.Categories,File.Emotions);
C = get(handles.error_table, 'data');
C(5,1)= num2cell(Normal_Error_Rate(1,1));%assign a cell array
...
% --- Executes on button press in features2_analysis_btn.
function features2_analysis_btn_Callback(hObject, eventdata, handles)
% hObject    handle to features2_analysis_btn (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% hObject    handle to hybrid_analysis_btn (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
File=load('Features2_Results');
C = get(handles.algo_category_table, 'data');
C(5,1)= num2cell(File.Category_Error_Rate);%assign a cell array
C(5,2)= num2cell(File.Category_Accuracy_Rate);
set(handles.algo_category_table, 'data', C);
...
% --- Executes on button press in features4_analysis_btn.
function features4_analysis_btn_Callback(hObject, eventdata, handles)
% hObject    handle to features4_analysis_btn (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% hObject    handle to hybrid_analysis_btn (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
File=load('Features3_Results');
C = get(handles.algo_category_table, 'data');
C(5,1)= num2cell(File.Category_Error_Rate);%assign a cell array
C(5,2)= num2cell(File.Category_Accuracy_Rate);
set(handles.algo_category_table, 'data', C);
...
% --------------------------------------------------------------------
function activex8_Error(hObject, eventdata, handles)
% hObject    handle to activex8 (see GCBO)
% eventdata  structure with parameters passed to COM event listener
% handles    structure with handles and user data (see GUIDATA)


% --- Executes on button press in nbtrainbtn.
function nbtrainbtn_Callback(hObject, eventdata, handles)
% hObject    handle to nbtrainbtn (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
[Status] = nc_training();
set(handles.training_progress_text,'string',Status);
...
% --- Executes on button press in nbtestallbtn.
function nbtestallbtn_Callback(hObject, eventdata, handles)
% hObject    handle to nbtestallbtn (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

[Category_Error_Rate,Category_Accuracy_Rate]= NB_Test();
C = get(handles.algo_category_table, 'data');
C(1,3)= num2cell(Category_Error_Rate(1));%assign a cell array
C(1,4)= num2cell(Category_Accuracy_Rate(1));
C(2,3)= num2cell(Category_Error_Rate(2));%assign a cell array
C(2,4)= num2cell(Category_Accuracy_Rate(2));
C(3,3)= num2cell(Category_Error_Rate(3));%assign a cell array
C(3,4)= num2cell(Category_Accuracy_Rate(3));
C(4,3)= num2cell(Category_Error_Rate(4));%assign a cell array
C(4,4)= num2cell(Category_Accuracy_Rate(4));
C(5,3)= num2cell(Category_Error_Rate(5));%assign a cell array
C(5,4)= num2cell(Category_Accuracy_Rate(5));
C(6,3)= num2cell(Category_Error_Rate(6));%assign a cell array
C(6,4)= num2cell(Category_Accuracy_Rate(6));
set(handles.algo_category_table, 'data', C);


% --- Executes on button press in nbtestbtn.
function nbtestbtn_Callback(hObject, eventdata, handles)
% hObject    handle to nbtestbtn (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% handles    structure with handles and user data (see GUIDATA)
wav_file=handles.sample_filepath_txt.String;
[Category,Category_score,Categories_error,Category_cost] = nb_classification(wav_file);
C = get(handles.test_table, 'data');


C(1,2)= Category(1);%assign a cell array
C(2,2)= Category(2);%assign a cell array
C(3,2)= Category(3);%assign a cell array
C(4,2)= Category(4);%assign a cell array
C(5,2)= Category(5);%assign a cell array
C(6,2)= Category(6);%assign a cell array
set(handles.test_table, 'data', C);
axes(handles.spectral_graph)
bar3(Category_score(1,:));
zlabel(' Naive Bayes Score');
xlabel(' Categories from 1 to 49'); 
title('Spectral Analysis Result');
axes(handles.frequency_graph)
bar3(Category_score(2,:));
